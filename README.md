...
- [FPrint](https://fprint.freedesktop.org/) - интеграция сканеров отпечатков пальцев. По отпечатку можно входить в систему или  использовать вместо пароля для `sudo` в Linux (не забудьте подключить PAM-модуль `sudo pam-auth-update`) и тп.
