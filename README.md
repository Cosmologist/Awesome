...
#### Блокировка парсера JavaScript'ом
...
> Cкрипт может захотеть получить доступ к предшествующим HTML и стилям или даже как-то преобразовать их. Поэтому браузер приостанавливает парсинг узлов, завершает построение CSSOM-дерева, исполняет скрипт (предварительно загружает, если скрипт внешний), а затем возобновляет парсинг. Поэтому говорят, что JavaScript блокирует парсинг. Такой режим работы является синхронным.
#### Как избежать блокировки страницы JavaScript'ом
Использовать асинхронную загрузку скриптов через аттрибуты тега *async* и *defer*, что позволяет избежать блокировки.
#### Чем отличаются аттрибуты async и defer
- *async* - скрипт загружается и выполняется сразу после его загрузки. Порядок выполнения никак не связан с порядком в котором скрипты встречаются на странице
- *defer* - аналогичен *async* кроме того, что выполняется после завершения загрузки *DOM* (событие *DOMDocumentLoaded*) с соблюдением последовательности.
#### Когда использовать аттрибут async, а когда defer
- *async* для скриптов, которые не работают с *DOM* (так как неизвестно, какие элементы *DOM* будут загружены к моменту окончания загрузки и запуска скрипта) и не имеют зависимостей от других скриптов на странице (потому что скрипт может загрузиться и запуститься раньше, чем скрипт от которого он зависит). **Примеры применения: счетчики, скрипты аналитики** и тд.
- *defer* для всех остальных случаев, кроме редких случаев, когда требуется запускать скрипт раньше чем будет загружен *DOM*.
