...
- [FPrint](https://fprint.freedesktop.org/) поддержка сканеров отпечатков пальцев. По отпечатку можно входить в систему или  использовать вместо для `sudo` в Linux (не забудьте подключить PAM-модуль `sudo pam-auth-update`) и тп.
